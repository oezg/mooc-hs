...
treeMax Empty = 0
treeMax (Node val l r) = maximum [val, treeMax l, treeMax r]
...
search goal tree = helper [] tree
  where
    helper _ Empty = Nothing
    helper acc (Node val l r)
      | goal == val = Just (reverse acc)
      | otherwise = case helper (StepL : acc) l of
          Nothing -> helper (StepR : acc) r
          m -> m
