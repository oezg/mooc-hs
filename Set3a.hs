...
capitalizeFirst string = (toUpper $ head string) : tail string
...
powers :: Int -> Int -> [Int]
powers k max = powers' [] 0 k max

powers' acc cur k max =
  if k ^ cur > max
    then reverse acc
    else powers' (k ^ cur : acc) (cur + 1) k max
...
joinToLength n strings =
  [string1 ++ string2 | string1 <- strings, string2 <- strings, length string1 + length string2 == n]
...
multiCompose' acc (f : fs) = multiCompose' (acc . f) fs
...
interpreter' x y acc [] = reverse acc
interpreter' x y acc (c : cs) =
  case c of
    "up" -> interpreter' x (y + 1) acc cs
    "down" -> interpreter' x (y - 1) acc cs
    "left" -> interpreter' (x - 1) y acc cs
    "right" -> interpreter' (x + 1) y acc cs
    "printX" -> interpreter' x y (show x : acc) cs
    "printY" -> interpreter' x y (show y : acc) cs
