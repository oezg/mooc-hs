...
longest' :: (Ord a) => [[a]] -> [a]
longest' lists =
  case sortBy customSort lists of
    x : xs -> x
  where
    customSort e1 e2
      | compare (length e1) (length e2) == EQ = compare (head e1) (head e2)
      | otherwise = case compare (length e1) (length e2) of
          LT -> GT
          _ -> LT
...
longest (x : xs) = helper x xs
  where
    helper acc [] = acc
    helper acc (x : xs)
      | length x > length acc = helper x xs
      | length x < length acc = helper acc xs
      | head x < head acc = helper x xs
...
freqs = foldr (Map.alter altering) Map.empty
  where
    altering Nothing = Just 1
    altering (Just y) = Just (y + 1)
