...
simplify (RationalNumber a b) =
  let n = gcd a b
   in RationalNumber (div a n) (div b n)
...
instance Num RationalNumber where
  ...
  abs (RationalNumber c d) = RationalNumber (abs c) d
  signum :: RationalNumber -> RationalNumber
  signum (RationalNumber c d) =
    if c < 0 then RationalNumber (-1) 1 else RationalNumber 1 1
  fromInteger :: Integer -> RationalNumber
  fromInteger x = RationalNumber x 1
  negate :: RationalNumber -> RationalNumber
  negate (RationalNumber c d) = RationalNumber (-c) d
