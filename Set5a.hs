...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix color1 color2) =
  [color1, color2]
    |> foldr (\c s -> zipWith (+) s (map (/ 2) $ rgb c)) [0, 0, 0]
rgb (Invert color) = map (flip subtract 1) $ rgb color
...
fromBin b = fromBin' 0 b
  where
    fromBin' :: Int -> Bin -> Int
    fromBin' _ End = 0
    fromBin' exp (O bin) = fromBin' (exp + 1) bin
    fromBin' exp (I bin) = (2 ^ exp) + fromBin' (exp + 1) bin
