...
sums i = helper 1
  where
    ...
    helper n
      | n == i = [littleGauss i]
      | otherwise = littleGauss n : helper (n + 1)

    littleGauss :: Int -> Int
    littleGauss n = div (n * (n + 1)) 2
...
mymaximum bigger initial xs = helper initial xs
  where
    helper acc [] = acc
    helper acc (curr : rest)
      | bigger curr acc = helper curr rest
      | otherwise = helper acc rest
